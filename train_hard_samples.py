...
def default_loader(path):
    try:
        ...
        with open('read_error_3.txt', 'a') as fid:
...
class BatchDataset(Dataset): #产生训练集
    def __init__(self, transform=None, dataloader=default_loader):
        ...
        with open(rootPath+'/all/train3.txt', 'r') as fid:
            self.imglist = fid.readlines()
...
def main():
    ...
    train_loader = torch.utils.data.DataLoader(
        train_dataset, num_workers=num_workers, batch_size=batchsize, pin_memory=True, shuffle=True)
    ...
    for e in range(epoch):
        ...
        # if (e + 1) % 10 == 0:
        polt_curve(e+1, train_Accuracy_list, train_Loss_list, test_Accuracy_list, test_Loss_list,namePostfix)
        result = open('./models_pkl/result3_3_' + namePostfix + '.txt', 'w')
        for index in range(e+1):
            result.write('{}|{}|{}|{}|{}\n'.format(index + 1, train_Accuracy_list[index], train_Loss_list[index],
                                                   test_Accuracy_list[index], test_Loss_list[index]))
        result.close()
...
def val(val_loader, net, criterion,e,epoch):
    ...
    # f1 = open('./test_negative3_3_' + namePostfix + '.txt', 'w')
    net.eval()
    with torch.no_grad():
...
def polt_curve(epoch,tarin_Accuracy_list,tarin_Loss_list,test_Accuracy_list,test_Loss_list,namePostfix):
    plt.clf()
    x1 = range(0, epoch)
    x2 = range(0, epoch)
    ...
    plt.subplot(2, 1, 1)
    plt.plot(x1, y1, 'g-',label='train3')
    plt.plot(x1, y3, 'r-',label='test3')
    ...
    plt.subplot(2, 1, 2)
    plt.plot(x2, y2, 'go-',label='train3')
    plt.plot(x2, y4, 'rs-',label='test3')
    plt.xlabel('loss vs. epoches')
    plt.xlabel('epoch')
    plt.ylabel('loss')
    plt.legend()

    plt.savefig('./train3_3_' + namePostfix + '.jpg')
    # plt.show()
