...
def default_loader(path):
    try:
        ...
        with open('read_error_1.txt', 'a') as fid:
            fid.write(path + ',,,' + time.asctime(time.localtime(time.time())) + '\n')
        return Image.new('RGB', (224,224), 'white')
    return img

class RandomDataset1(Dataset): #产生测试集/验证集
...
class BatchDataset(Dataset): #产生训练集
    def __init__(self, transform=None, dataloader=default_loader):
        ...
        with open(rootPath+'/all/train1.txt', 'r') as fid:
            self.imglist = fid.readlines()
    ...
    for e in range(epoch):
        ...
        for i, (inputs, targets) in enumerate(train_loader):
            ...
            if i == len(train_loader) - 1:
                val_dataset1 = RandomDataset1(transform=transforms.Compose([
                ...
                val_loader1 = torch.utils.data.DataLoader(
                    val_dataset1, batch_size=batchsize, shuffle=True,
                    num_workers=4, pin_memory=True)

                prec1,test_Loss = val(val_loader1, net, criterion,e,epoch,namePostfix)
                test_Loss_list1.append(test_Loss)
                test_Accuracy_list1.append(prec1)
                ...
                prec12, test_Loss2 = val2(val_loader2, net, criterion, e, epoch,namePostfix)
                test_Loss_list2.append(test_Loss2)
                test_Accuracy_list2.append(prec12)

        # f1.close()
        train_Loss_list.append(softmax_losses.avg)
        train_Accuracy_list.append(top1.avg)
        # if (e + 1) % 10 == 0:
        polt_curve(e+1, train_Accuracy_list, train_Loss_list, test_Accuracy_list1, test_Loss_list1,
                   test_Accuracy_list2, test_Loss_list2,namePostfix)
        result = open('./models_pkl/result1_1_2_'+namePostfix+'.txt', 'w')
        for index in range(e+1):
            result.write(
                '{}|{}|{}|{}|{}|{}|{}\n'.format(index + 1, train_Accuracy_list[index], train_Loss_list[index],
                                                test_Accuracy_list1[index], test_Loss_list1[index],
...
def val(val_loader, net, criterion,e,epoch,namePostfix):
    batch_time = AverageMeter()
    softmax_losses = AverageMeter()
    top1 = AverageMeter()
    ...
    # f1 = open('./test_negative1_1_'+namePostfix+'.txt', 'w')
    net.eval()
    with torch.no_grad():
...
def val2(val_loader, net, criterion,e,epoch,namePostfix):
    ...
    # net = ResNeSt().to(device)
    # f1 = open('F:\SoftwareProgram\Data\dataset\snow leopard\original\\test_negative.txt', 'w')
    # f1 = open('./test_negative1_2_'+namePostfix+'.txt', 'w')
    net.eval()
    with torch.no_grad():
        for i, (inputs, targets,image_paths) in enumerate(val_loader):
...
def polt_curve(epoch,tarin_Accuracy_list,tarin_Loss_list,test_Accuracy_list1,test_Loss_list1,test_Accuracy_list2,test_Loss_list2,namePostfix):
    plt.clf()
    x1 = range(0, epoch)
    x2 = range(0, epoch)
    y1 = tarin_Accuracy_list
    y2 = tarin_Loss_list
    y3 = test_Accuracy_list1
    y4 = test_Loss_list1
    y5 = test_Accuracy_list2
    y6 = test_Loss_list2

    plt.subplot(2, 1, 1)
    plt.plot(x1, y1, 'g-',label='train1')
    plt.plot(x1, y3, 'r-',label='test1')
    plt.plot(x1, y5, 'b-', label='test2')
    ...
    plt.plot(x2, y6, 'b-', label='test2')
    plt.xlabel('loss vs. epoches')
    plt.xlabel('epoch')
    plt.ylabel('loss')
    plt.legend()

    plt.savefig('./train1_1_2_'+namePostfix+'.jpg')
    # plt.show()
